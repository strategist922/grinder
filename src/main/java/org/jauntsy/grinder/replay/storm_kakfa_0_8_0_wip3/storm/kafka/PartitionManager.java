...
import org.jauntsy.grinder.replay.storm_kakfa_0_8_0_wip3.storm.kafka.KafkaSpout.EmitState;
import org.jauntsy.grinder.replay.storm_kakfa_0_8_0_wip3.storm.kafka.KafkaSpout.MessageAndRealOffset;

public class PartitionManager {
    public static final Logger LOG = Logger.getLogger(PartitionManager.class);

    static public class KafkaMessageId {
    ...
    public EmitState next(SpoutOutputCollector collector) {
        ...
        while(true) {
            ...
            List<Object> tup = _spoutConfig.kafkaScheme.deserialize(toEmit.topic, toEmit.hostname, toEmit.port, toEmit.partition, toEmit.offset, toEmit.nextOffset, Utils.toByteArray(toEmit.msg.payload()));
            if(tup!=null) {
...
//        LOG.info("Fetching from Kafka: " + _consumer.host() + ":" + _partition.partition + " from offset " + _emittedToOffset);
        ...
        if(numMessages>0) {
          LOG.info("Fetched " + numMessages + " messages from Kafka: " + _consumer.host() + ":" + _partition.partition + ":" + _emittedToOffset);
        }
        for(MessageAndOffset msg: msgs) {
            _pending.add(_emittedToOffset);
            _waitingToEmit.add(new MessageAndRealOffset(msg.message(), _spoutConfig.topic, _partition.host.host, _partition.host.port, _partition.partition, _emittedToOffset, msg.offset()));
...
//        LOG.warn("+++++PartitionManager.ack: " + _spoutConfig.topic + ":" + _partition.partition + ":" + offset + "+++++");
    ...
    public void fail(Long offset) {
        ...
        LOG.warn("PartitionManager.fail: " + _spoutConfig.topic + ":" + _partition.partition + ":" + offset);
    ...
    public void commit() {
        LOG.info("Committing offset for " + _spoutConfig.id + "/" + _partition);
        long committedTo;
        if(_pending.isEmpty()) {
        ...
        LOG.info("Comitted offset for " + _spoutConfig.id + "/" + _partition);
    }
