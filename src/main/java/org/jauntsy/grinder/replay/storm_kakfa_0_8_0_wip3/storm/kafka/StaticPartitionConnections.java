...
public class StaticPartitionConnections {
    ...
    public HostPort getHost(int partition) {
        int hostIndex = partition / hosts.partitionsPerHost;
        return hosts.hosts.get(hostIndex);
    }

    public HostPortPartition getHostPortPartition(int partition) {
        return new HostPortPartition(getHost(partition), partition % hosts.partitionsPerHost);
    }
