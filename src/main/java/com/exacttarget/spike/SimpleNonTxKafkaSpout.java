 ...
 * Time: 11:25 AM
 */
public class SimpleNonTxKafkaSpout extends BaseRichSpout {
    ...
    public SimpleNonTxKafkaSpout(String host, int port, int soTimeout, int bufferSize, String topic, int partition) {
        this.host = host;
        this.port = port;
        ...
        try {
            if (pending.size() < 10000) {
                ...
                for (MessageAndOffset mno : fetch) {
                    Long id = mno.offset();
                    pending.add(id);
                    collector.emit(new Values(new String(KafkaUtil.copyBytes(mno.message()))));
                    offset = mno.offset();
                }
